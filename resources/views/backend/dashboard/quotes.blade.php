			...
			<table class="table table-striped table-hover">
				...
				<tbody id="tasktablebody">
					...
					@foreach($quote as $quote)
						...
						<td>{{$quote->id}}</td>
						<td>{{$quote->quotes_bn}}</td>
						<td>{{$quote->quotes_en}}</td>
						
						<td>
							<a href="#editEmployeeModal" class="edit" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a>
							<a href="{{asset('quote/deletedata/'.$quote->id)}}" onclick="return myConfirm();" class="delete"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a>
						</td>
					</tr>
					@endforeach
			...
			<form name="" action="{{asset('/quote/store')}}" method="post">
				...
				<div id="createTaskMessage"></div>					
					<div class="form-group">
						<label>Quotes_bn</label>
						<input type="text" name="quotes_bn" class="form-control" required>
					</div>
					
					<div class="form-group">
						<label>Quotes_en</label>
						<textarea  name="quotes_en" rows="4" cols="40"></textarea>
					</div>
